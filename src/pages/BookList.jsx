...
const BookList = () => {
  ...
  // useFetchBooks ya carga los datos en su propio useEffect
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      ...
      if (success) {
        ...
        getBooks(); // Refrescar la lista tras eliminar
      } else {
          ...
          <div className="w-full overflow-x-auto bg-gray-800 shadow-lg rounded-lg">
            <table className="min-w-full divide-y divide-gray-700">
              <thead className="bg-gray-700">
                <tr>
                  <th className="px-6 py-3 text-left text-sm font-semibold text-gray-300 uppercase tracking-wider">TÍTULO</th>
                  <th className="px-6 py-3 text-left text-sm font-semibold text-gray-300 uppercase tracking-wider">AUTOR</th>
                  <th className="px-6 py-3 text-left text-sm font-semibold text-gray-300 uppercase tracking-wider">GÉNERO</th>
                  <th className="px-6 py-3 text-center text-sm font-semibold text-gray-300 uppercase tracking-wider">ESTADO</th>
                  <th className="px-6 py-3 text-center text-sm font-semibold text-gray-300 uppercase tracking-wider">ACCIONES</th>
                </tr>
              </thead>
              <tbody className="bg-gray-800 divide-y divide-gray-700">
                {books.map((book) => (
                  <tr key={book.id} className="hover:bg-gray-700">
                    <td className="px-6 py-4 whitespace-nowrap text-gray-100">{book.libro}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-100">{book.autor}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-100">{book.genero}</td>
